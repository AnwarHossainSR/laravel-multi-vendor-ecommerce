            ...
            <div class="card mt-5">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <h3 class="display-6">Add post</h3>
                    </div>
                                ...
                                <div class="col-sm-12">
                                    <select name="post_cat_id" id="cat_id" class="form-control mb-3 mb-3">
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" {{ old('post_cat_id') }}>{{ $category->title }}</option>
                                        @endforeach
                                    </select>
                                  @error('post_cat_id')
                                    <span class="text-danger">{{$message}}</span>
                                ...
                                <div class="col-sm-12">
                                    <select name="added_by" class="form-control">
                                        ...
                                        @foreach($users as $key=>$data)
                                          <option value='{{$data->id}}' {{($data->id==Auth::user()->id) ? 'selected' : ''}}>{{$data->full_name}}</option>
                                      @endforeach
                                    </select>
                                  @error('added_by')
                                    <span class="text-danger">{{$message}}</span>
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <div class="form-group-material">
                              ...
                              <div class="col-lg-12">
                                  <textarea id="quote" name="quote" class="input-material summary" style="background-color:#22252A;border:1px solid #62666C">{{ old('quote') }}</textarea>
                                  @error('quote')
                                      <span class="text-danger">{{$message}}</span>
                                  @enderror
                              </div>
