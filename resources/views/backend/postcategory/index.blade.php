            ...
            <div class="card mt-5">
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        <h1 class="display-4">All Post Category</h1>
                        <h1 class="display-4"><a href="{{ route('postcategory.create') }}" class="btn btn-secondary text-white p-2"><i class="fa fa-plus" aria-hidden="true"></i> Add Post Tag</a></h1>
                    </div>
                    <table id="table" class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="checkbox" name="toggle" value="{{ $postcategory->id }}" data-toggle="switchbutton" {{ $postcategory->status == 'active' ? 'checked':'' }} data-onlabel="active" data-offlabel="inactive"data-size="sm" data-onstyle="success" data-offstyle="danger">
                                </td>
                                <td>
                                    <a href="{{route('postcategory.edit',$postcategory->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fa fa-edit"></i></a>
                                    <form method="POST" action="{{route('postcategory.destroy',[$postcategory->id])}}">
                                    @csrf
                                    @method('delete')
                                        <button class="btn btn-danger btn-sm dltBtn" data-id={{$postcategory->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fa fa-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
