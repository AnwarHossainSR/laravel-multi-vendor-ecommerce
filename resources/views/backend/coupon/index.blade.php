...
<div class="page-content">
	<div class="page-header">
	  <div class="container-fluid">
		<h2 class="h5 no-margin-bottom"><i class="fa fa-home" aria-hidden="true"></i>&nbsp;<a href="{{ route('coupon.index') }}">Coupons</a></h2>
	  </div>
	</div>
            ...
            <div class="card mt-5">
                <div class="card-body"z>
                    <div class="d-flex justify-content-between">
                        <h1 class="display-4">All Coupon</h1>
                        <h1 class="display-4"><a href="{{ route('coupon.create') }}" class="btn btn-secondary text-white p-2"><i class="fa fa-plus" aria-hidden="true"></i> Create Coupon</a></h1>
                    </div>
                    <table id="table" class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$coupon->value}}</td>
                                <td>
                                    <input type="checkbox" name="toggle" value="{{ $coupon->id }}" data-toggle="switchbutton" {{ $coupon->status == 'active' ? 'checked':'' }} data-onlabel="active" data-offlabel="inactive"data-size="sm" data-onstyle="success" data-offstyle="danger">
                                </td>

                                <td>
                                    <a href="{{route('coupon.edit',$coupon->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fa fa-edit"></i></a>
                                    <form method="POST" action="{{route('coupon.destroy',[$coupon->id])}}">
                                    @csrf
                                    @method('delete')
                                        <button class="btn btn-danger btn-sm dltBtn" data-id={{$coupon->id}} style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" data-placement="bottom" title="Delete"><i class="fa fa-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
